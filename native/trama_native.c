...
static ExecResult import_module(Program *current_program, Value module_ref) {
    ...
    char *candidate = NULL;
    if (strstr(ref, ".tbc") != NULL) {
        candidate = strdup(ref);
    } else if (strstr(ref, ".trm") != NULL) {
        size_t n = strlen(ref);
        if (n >= 4) n -= 4;
        candidate = (char *)malloc(n + 5);
        if (!candidate) fatal("sem memória");
        memcpy(candidate, ref, n);
        memcpy(candidate + n, ".tbc", 5);
    } else {
        size_t n = strlen(ref);
        candidate = (char *)malloc(n + 5);
        if (!candidate) fatal("sem memória");
...
static ExecResult run_function(Program *p, Function *start, Value *args, size_t argc, Env *closure) {
    ...
    while (1) {
        ...
        if (strcmp(in->op, "LOAD_CONST") == 0) {
            ...
            Value v = frame_pop(&cur);
            if (v.type != VAL_AWAITABLE) fatal("valor não aguardável em AWAIT");
            Awaitable *a = v.as.awaitable;
            ExecResult ar = run_function(p, a->fn, a->args, a->argc, a->closure);
            if (ar.threw) {
...
static int run_tbc(const char *path) {
    ...
    if (env_get(globals, "principal", &principal) && principal.type == VAL_FUNC) {
        ...
        if (pcall.value.type == VAL_AWAITABLE) {
            Awaitable *a = pcall.value.as.awaitable;
            ExecResult aw = run_function(&p, a->fn, a->args, a->argc, a->closure);
            if (aw.threw) {
...
static int run_via_standalone_cli(int argc, char **argv) {
    const char *cli = getenv("TRAMA_STANDALONE_CLI");
    if (!cli || cli[0] == '\0') cli = "trama";

    char **args = (char **)calloc((size_t)argc + 1, sizeof(char *));
    if (!args) fatal("sem memória");
    args[0] = (char *)cli;
...
static void usage(void) {
    ...
    puts("obs:");
    puts("  nesta etapa, executar/compilar usam ponte por binario standalone (sem invocar python diretamente).");
}

int main(int argc, char **argv) {
    if (argc >= 2 && strcmp(argv[1], "--diagnostico-runtime") == 0) {
        puts("backend_runtime=nativo_c_vm");
        puts("backend_compilador=nativo_hibrido_standalone");
        puts("requer_python_host=nao_quando_standalone");
        puts("runtime_backend=nativo_c_vm");
        puts("compilador_backend=nativo_hibrido_standalone");
        puts("python_host_required=nao_quando_standalone");
        return 0;
    }
    ...
    if (argc >= 2 && (strcmp(argv[1], "executar") == 0 || strcmp(argv[1], "run") == 0 || strcmp(argv[1], "compilar") == 0 || strcmp(argv[1], "compile") == 0)) {
        return run_via_standalone_cli(argc, argv);
    }
