# Manual Completo da linguagem Trama (até v1.4)

Este manual consolida os recursos entregues da `v0.1` até a `v1.4`.
...
## 10. v1.2 - Uploads, mídia e storage

### 10.1 Upload multipart/form-data

- parsing multipart no runtime web;
...
### 10.2 Pipeline de mídia

- `midia_ler_arquivo`, `midia_salvar_arquivo`;
- `midia_comprimir_gzip`, `midia_descomprimir_gzip`;
- `midia_sha256`;
- `midia_redimensionar_imagem`, `midia_converter_imagem`;
- `midia_pipeline`;
- aliases: `midia_gzip_comprimir`, `midia_gzip_descomprimir`.

### 10.3 Storage (local e S3-compatível)
...
- `id_requisicao`, `id_traco`, `id_usuario`
- aliases: `request_id`, `trace_id`, `user_id`

### Métricas de produção

- HTTP: `http.requisicoes_total`, `http.latencia_ms`, `http.erros_total`
- DB: `db.operacoes_total`, `db.latencia_ms`, `db.erros_total`
- Runtime: `runtime.eventos_total` (cache/resiliência/jobs/webhooks)

### Tracing por requisição

- span raiz `http.requisicao`;
- evento de resposta `http.resposta`;
- finalização com status.

### Dashboard e alertas

- ativação: `web_ativar_observabilidade` / `web_observabilidade_ativar`
- endpoints padrão:
  - `/observabilidade`
  - `/alertas`
- APIs:
